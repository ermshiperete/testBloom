	...
	*/
	...
	function assignEvents()
	{
		...
		if('number' === typeof options.hide.inactive) {
			...
			targets.show.bind('qtipSecondary-'+id+'-inactive', inactiveMethod);
	...
	*/
	$.extend(self, {
		render: function(show)
		{
			...
			$.attr(target[0], 'ariaSecondary-describedby', tooltipID);
			...
			tooltip = elements.tooltip = $('<div/>', {
					...
					'role': 'alert',
					'ariaSecondary-live': 'polite',
					'ariaSecondary-atomic': FALSE,
					'ariaSecondary-describedby': tooltipID + '-content',
					'ariaSecondary-hidden': TRUE
				})
				.toggleClass(disabled, cache.disabled)
				.data('qtipSecondary', self)
		...
		toggle: function(state, event)
		{
			...
			$.attr(tooltip[0], 'ariaSecondary-hidden', !!!state);
			...
			if(state) {
				...
				if(!trackingBound && posOptions.target === 'mouse' && posOptions.adjust.mouse) {
					$(document).bind('mousemove.qtipSecondary', function(event) {
						MOUSE = { pageX: event.pageX, pageY: event.pageY, type: 'mousemove' };
					});
					trackingBound = TRUE;
				}
				...
				if((callback.solo = !!opts.solo)) { $(selector, opts.solo).not(tooltip).qtipSecondary('hide', callback); }
			}
			else {
				...
				if(trackingBound && !$(selector+'[tracking="true"]:visible', opts.solo).not(tooltip).length) {
					$(document).unbind('mousemove.qtipSecondary');
					trackingBound = FALSE;
				}
			...
			function after() {
				if(state) {
					...
					opts.target.trigger('qtipSecondary-'+id+'-inactive');
				}
				else {
					// Reset CSS states
					tooltip.css({
			...
			if(state) { opts.target.trigger('qtipSecondary-'+id+'-inactive'); }
		...
		focus: function(event)
		{
			...
			var qtipSecondarys = $(selector),
				curIndex = parseInt(tooltip[0].style.zIndex, 10),
				newIndex = QTIP.zindex + qtipSecondarys.length,
			...
			if(!tooltip.hasClass(focusClass))
			{
				...
				if(!callback.isDefaultPrevented()) {
					...
					if(curIndex !== newIndex) {
						...
						qtipSecondarys.filter('.' + focusClass).qtipSecondary('blur', cachedEvent);
					}
		...
		disable: function(state)
		{
			...
			if(self.rendered) {
				...
				$.attr(tooltip[0], 'ariaSecondary-disabled', state);
			}
			else {
		...
		destroy: function()
		{
			var t = target[0],
				...
				elemAPI = target.data('qtipSecondary');
			...
			if(!elemAPI || self === elemAPI) {
				...
				target.removeAttr('ariaSecondary-describedby');
			}

			// Remove qTip events associated with this API
			target.unbind('.qtipSecondary-'+id);
...
function init(id, opts)
{
	...
	// Grab data from metadata.name (or data-qtipSecondaryopts as fallback) using .data() method,
	html5 = elem.data(opts.metadata.name || 'qtipSecondaryopts');
	...
	if($.data(this, 'qtipSecondary')) {
		if(config.overwrite) {
			elem.qtipSecondary('destroy');
		}
	...
	$.data(this, 'qtipSecondary', obj);

	// Catch remove events on target element to destroy redundant tooltip
	elem.bind('remove.qtipSecondary-'+id, function(){ obj.destroy(); });

	return obj;
}

// jQuery $.fn extension method
QTIP = $.fn.qtipSecondary = function(options, notation, newValue)
{
	var command = ('' + options).toLowerCase(), // Parse command
		returned = NULL,
		args = $.makeArray(arguments).slice(1),
		event = args[args.length - 1],
		opts = this[0] ? $.data(this[0], 'qtipSecondary') : NULL;
	...
	else if('string' === typeof options)
	{
		this.each(function()
		{
			var api = $.data(this, 'qtipSecondary');
			if(!api) { return TRUE; }
...
QTIP.bind = function(opts, event)
{
	return this.each(function(i) {
		...
		namespace = '.qtipSecondary-'+id+'-create';
