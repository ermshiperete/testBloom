 ...
 *
 * Date: Wed Jun 12 2013 04:21 UTC+0000
...
(function( factory ) {
	...
	else if(jQuery && !jQuery.fn.qtipSecondary) {
		factory(jQuery);
	}
}
(function($) {
	...
	var TRUE = true,
		...
		selector = 'div.qtipSecondary.'+NAMESPACE,
...
function QTip(target, options, id, attr)
{
	...
	function createTitle()
	{
		...
		.delegate('.qtipSecondary-close', 'mousedown keydown mouseup keyup mouseout', function(event) {
			$(this).toggleClass('ui-state-active ui-state-focus', event.type.substr(-4) === 'down');
		})
		.delegate('.qtipSecondary-close', 'mouseover mouseout', function(event){
		...
		toggle: function(state, event)
		{
			...
			if(state) {
				...
				if(!trackingBound && posOptions.target === 'mouse' && posOptions.adjust.mouse) {
					$(document).bind('mousemove.qtipSecondary', storeMouse);
					trackingBound = TRUE;
				}
				...
				if(!!opts.solo) {
					$(selector, opts.solo).not(tooltip).qtipSecondary('hide', $.Event('tooltipsolo'));
				}
			}
			else {
			...
			if(!tooltip.hasClass(focusClass))
			{
				...
				if(self._triggerEvent('focus', [newIndex], cachedEvent)) {
					...
					if(curIndex !== newIndex) {
						...
						qtips.filter('.' + focusClass).qtipSecondary('blur', cachedEvent);
					}
...
function init(id, opts)
{
	...
	elem.bind('remove.qtipSecondary-'+id+' removeqtip.qtipSecondary-'+id, function(){ obj.destroy(); });
...
PLUGINS = QTIP.plugins = {
	...
	offset: function(elem, container) {
		var pos = elem.offset(),
			...
			quirks = false && document.compatMode !== 'CSS1Compat',
...
function Modal(api)
{
	var self = this,
		...
		globalNamespace = '.qtipSecondarymodal',
	...
	function stealFocus(event) {
		var target = $(event.target),
			container = target.closest('.qtipSecondary'),
			targetOnTop;
	...
	$.extend(self, {
		init: function()
		{
			...
			.bind('tooltipfocus'+globalNamespace, function(event) {
				...
				// Fire blur event for focused tooltip
				qtips.end().filter('.' + focusClass).qtipSecondary('blur', event.originalEvent);
			...
			.bind('tooltiphide'+globalNamespace, function(event) {
				if(event.target === tooltip[0]) {
					$('[' + attr + ']').filter(':visible').not(tooltip).last().qtipSecondary('focus', event);
				}
			});
	...
	$.extend(self, {
		init: function()
		{
			...
			if(elems.overlay && !modalProcessed) {
				...
				win.bind('scroll.qtipSecondary-ie6, resize.qtipSecondary-ie6', scroll);
